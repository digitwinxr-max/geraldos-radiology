...
export async function POST(request: NextRequest) {
  ...
  if (!endpoint) {
    return NextResponse.json({ error: "MinIO is not configured (MINIO_ENDPOINT)" }, { status: 503 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "MinIO unreachable", detail: error instanceof Error ? error.message : String(error) },
      { status: 502 }
    );
  }
}
