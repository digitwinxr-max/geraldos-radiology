...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (!url) {
    return NextResponse.json({ error: "Orthanc is not configured (ORTHANC_URL)" }, { status: 503 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Orthanc unreachable", detail: error instanceof Error ? error.message : String(error) },
      { status: 502 }
    );
  }
}
