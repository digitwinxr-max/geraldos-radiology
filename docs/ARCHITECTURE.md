...
- **Viewer**: OHIF Web Viewer, reverse-proxied by the app at `/viewer` on its own origin (private service; `OHIF_URL` is server-side only). Deep links from study rows: `/viewer/viewer?StudyInstanceUIDs=<uid>`.
