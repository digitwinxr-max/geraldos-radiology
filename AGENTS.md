...
3. **DO NOT Leak Server Secrets**:
   - PACS credentials (`ORTHANC_PASSWORD`), Keycloak client secrets, and database strings must remain strictly on the server. Never pass raw credentials to the browser or expose them via `NEXT_PUBLIC_` prefixes.
...
2. **Preserve Fallback Modes**: LangGraph and Redis are designed with live PostgreSQL fallbacks. Always ensure offline/standalone operation remains functional when external services are disabled.
